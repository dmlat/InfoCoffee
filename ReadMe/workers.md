...
-   **Функциональность**:
    ...
    -   **Ежедневный импорт (23:05 МСК, Пн-Сб)**: Запускает более полный импорт (за 72 часа) для актуализации данных.
    -   **Еженедельный импорт (23:10 МСК, Воскресенье)**: Запускает импорт транзакций за последние 8 дней для полноты данных.
-   **Технические нюансы**:
...
-   **Расписание**: Запускается ежедневно в 4:00 по московскому времени.
-   **Техническая реализация**: Задача зарегистрирована в `schedule_imports.js` с использованием `node-cron`. Она вызывает функцию `checkPaymentStatus` из `payment_status_checker_worker.js`.
-   **Цель**: Автоматически вернуть в работу пользователей, у которых была проблема с оплатой подписки Vendista, но которые ее уже решили.
